...
export class NotificationJobComponent implements OnInit, OnChanges {
  ...
  ngOnInit() {
    ...
    this.db.list('job').snapshotChanges().map(actions => {
      ...
      this.job = job.filter((data: any) => data.value.status === 'notification' && data.value.statusDelete !== 'delete');
      // this.job = job
  ...
  selectValue(driver, data) {
    if (driver.target.value !== data.value.driver) {
      console.log('data', data, driver.target.value);
      this.checkSelectedDriver = data.key
      let dataDriver
      this.db.list('wikis').snapshotChanges().map(action => {
        return action.map(action => ({ key: action.key, value: action.payload.val() }));
      }).subscribe(wikis => {
        this.wikis2 = wikis.filter((data: any) => data.value.firstname === driver.target.value);
        dataDriver = this.wikis2[0];

        this.selectedEmailDriver = dataDriver.value.email;
        this.selectedNameDriver = dataDriver.value.firstname;
        this.selectedTokenDriver = dataDriver.value.token;
      });
    } else
      this.checkSelectedDriver = '';
  ...
  openDataDriver(con, data) {
    console.log('showdataDialog');
